...
#elif defined(USE_ARDUINO_MKR_PIN_LAYOUT) || defined(ARDUINO_SAMD_MKRZERO) || \
    defined(ARDUINO_SAMD_MKR1000) || defined(ARDUINO_SAMD_MKRFox1200) ||      \
...
uint8_t W5100Class::init(void) {
  ...
  // reset its SPIETH state when CS goes high (inactive).  Communication
  ...
  if (isW5200()) {
    ...
    // SPIETH well with this chip.  It appears to be very resilient,
    ...
    // that's heard other SPIETH communication if its chip select
    // pin wasn't high when a SD card or other SPIETH chip was used.
  } else {
