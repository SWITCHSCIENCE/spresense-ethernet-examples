...
enum W5100Linkstatus { UNKNOWN, LINK_ON, LINK_OFF };
...
#define __GP_REGISTER8(name, address)                                      \
  static inline void write##name(uint8_t _data) { write(address, _data); } \
  static inline uint8_t read##name() { return read(address); }
...
#define __GP_REGISTER_N(name, address, size)          \
 ...
 public:
  ...
  __GP_REGISTER_N(UIPR, 0x002A,
                  4);  // Unreachable IP address in UDP mode (W5100 only)
  __GP_REGISTER16(UPORT,
                  0x002E);  // Unreachable Port address in UDP mode (W5100 only)
  ...
  __GP_REGISTER8(PHYCFGR_W5500,
                 0x002E);  // PHY Configuration register, default: 10111xxx
...
#undef __GP_REGISTER_N
 ...
 private:
  ...
  static inline uint16_t readSn(SOCKET s, uint16_t addr, uint8_t *buf,
                                uint16_t len) {
    return read(CH_BASE() + s * CH_SIZE + addr, buf, len);
  }
  static inline uint16_t writeSn(SOCKET s, uint16_t addr, uint8_t *buf,
                                 uint16_t len) {
...
#define __SOCKET_REGISTER8(name, address)                    \
  ...
  static inline uint8_t read##name(SOCKET _s) { return readSn(_s, address); }
...
#undef __SOCKET_REGISTER_N
 ...
 private:
  ...
  inline static void setSS() { *(ss_pin_reg) &= ~ss_pin_mask; }
  inline static void resetSS() { *(ss_pin_reg) |= ss_pin_mask; }
  ...
  inline static void setSS() { *(ss_pin_reg + 256) = 1; }
  inline static void resetSS() { *(ss_pin_reg + 128) = 1; }
  ...
  inline static void setSS() { *(ss_pin_reg + 8) = ss_pin_mask; }
  inline static void resetSS() { *(ss_pin_reg + 4) = ss_pin_mask; }
  ...
  inline static void setSS() { *(ss_pin_reg + 13) = ss_pin_mask; }
  inline static void resetSS() { *(ss_pin_reg + 12) = ss_pin_mask; }
  ...
  inline static void setSS() { *(ss_pin_reg + 8 + 1) = ss_pin_mask; }
  inline static void resetSS() { *(ss_pin_reg + 8 + 2) = ss_pin_mask; }
  ...
  inline static void setSS() { GPOC = ss_pin_mask; }
  inline static void resetSS() { GPOS = ss_pin_mask; }
  ...
  inline static void setSS() { *(ss_pin_reg + 5) = ss_pin_mask; }
  inline static void resetSS() { *(ss_pin_reg + 6) = ss_pin_mask; }

#elif defined(ARDUINO_ARCH_SPRESENSE)
  inline static void initSS() {
    if (ss_pin != 10) pinMode(ss_pin, OUTPUT);
  }
  inline static void setSS() {
    if (ss_pin != 10) digitalWrite(ss_pin, LOW);
  }
  inline static void resetSS() {
    if (ss_pin != 10) digitalWrite(ss_pin, HIGH);
  }
